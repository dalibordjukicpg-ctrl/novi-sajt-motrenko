...
function attachmentErrorLabel(
  code: BookingAttachmentValidationError,
  labels: ReturnType<typeof getBookingIntakeLabels>,
): string {
  switch (code) {
    case "tooMany":
      return labels.attachmentsErrorTooMany;
    case "tooLarge":
      return labels.attachmentsErrorTooLarge;
    case "totalTooLarge":
      return labels.attachmentsErrorTotalTooLarge;
    default:
      return labels.attachmentsErrorType;
  }
}
...
export async function processBookingSubmission(
  ...
  const ip = meta?.ip?.slice(0, 45) ?? null;
  ...
  try {
    ...
    for (const att of savedAttachments) {
      ...
      try {
        ...
        console.warn("[booking] attachment read for email", att.storageKey, e);
      }
    }
