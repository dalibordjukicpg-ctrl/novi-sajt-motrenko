...
function textOverlaySvg(copy) {
  ...
  return Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${OG_W}" height="${OG_H}">
    ...
    <text x="600" y="512" text-anchor="middle" font-family="Segoe UI, Arial, sans-serif" font-size="24" fill="#5c4a3a" letter-spacing="0.6">${esc(copy.tagline)}</text>
    <text x="600" y="568" text-anchor="middle" font-family="Georgia, 'Times New Roman', serif" font-size="58" font-style="italic" fill="${BRAND_ORANGE}" letter-spacing="3">${esc(copy.city)}</text>
  </svg>`);
}
