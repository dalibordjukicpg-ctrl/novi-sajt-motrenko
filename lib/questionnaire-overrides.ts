 ...
 * Defaulti iz `lib/questionnaire-i18n.ts`; admin ručno upisuje overrides.
 *
 * Skladišti se kao JSON u `site_locale_strings` pod ključem `questionnaire.full`.
 */
...
export async function getQuestionnaireI18nMerged(locale: Locale): Promise<QuestionnaireI18n> {
  ...
  const merged = override
    ? (deepMerge(
      ...
      ) as unknown as QuestionnaireI18n)
    : base;
